"""
Streamlit frontend for the Blogforge AI.
"""
...
from blogforge_ai.utils import safe_slug
...
def get_blogforge_app():
    ...
    try:
        from blogforge_ai.graph.pipeline import get_blogforge_app as _get_blogforge_app
        return _get_blogforge_app()
    except FileNotFoundError as e:
        st.error(f"⚠️ Config file not found:\n{e}")
        return None
    except Exception as e:
        st.error(
            f"⚠️ Blog Forge App failed to initialize:\n\n{e}\n\n"
            f"Check logs for details."
        )
        return None 

app = get_blogforge_app()
if app is None:
    st.stop()
...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    ...
    try:
        ...
        logger.exception(f"Exception occured while invoking the LLM")
        yield ("error", e)
    except Exception:
        logger.exception(f"Exception occured while invoking the LLM")
...
def read_md_file(path_of_file: Path) -> str:
    """
    Read the content of markdown file at the given location.
    Returns the content of file as string.
    """
    return path_of_file.read_text(encoding="utf-8", errors="replace")
...
def extract_title_from_md(md: str, fallback: str) -> str:
    ...
    Args:
        ...
        fallback:   Value need to return if no Heading is present
    """
    for line in md.splitlines():
        if line.startswith("# "):
            t = line[2:].strip() # Removes the "# " from start
...
st.set_page_config(page_title="Blog Forge AI", layout="wide")
...
with st.sidebar:
    ...
    if not past_files:
        ...
        for file in past_files[:50]:
            try:
                md_text = read_md_file(file)
                title = extract_title_from_md(md_text, file.stem)
                # file.stem >> returns the filename without its extension
            except Exception:
                title = file.stem
            label = f"{title}  -  {file.name}"
            options.append(label)
            file_by_label[label] = file
